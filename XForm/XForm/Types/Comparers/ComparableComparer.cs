...
{
    /// <summary>
    ///  IDataBatchComparer for T[].
    /// </summary>
    internal class ComparableComparer<T> : IDataBatchComparer, IDataBatchComparer<T> where T : System.IComparable<T>
    {
        ...
        public void GetHashCodes(DataBatch batch, int[] hashes)
        {
            ...
            for (int i = 0; i < batch.Count; ++i)
            {
                ...
                {
                    hashes[i] = (hashes[i] << 5) - hashes[i] + unchecked((int)Hashing.Hash(array[batch.Index(i)].GetHashCode(), 0));
                }
            }
        }
